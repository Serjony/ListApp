    ...
    public class DoubleLinkedList
    {
        ...
        public void AddValueByIndex(int value, int index)
        {
            if (index >= 0 && index <= Length)
            {
                if (Length != 0)
                {
                    ...
                    {
                        DoubleNode nodeByIndex = new DoubleNode(value);
        ...
        public void RemovNElementsFromLast(int nvalue)
        {
            if (nvalue < Length)
            {
                if (!(nvalue < 0))
                {
                    DoubleNode current = GetNodeByIndex(Length - nvalue);
                    current.Next = _tail;
                    _tail.Next = null;
        ...
        public void RemovNElementsFromStart(int nvalue)
        {
            if (nvalue < Length)
            {
                if (!(nvalue < 0))
                {
                    DoubleNode current = GetNodeByIndex(nvalue - 1);
                    _root = current.Next;
        ...
        public void RemoveByIndexNElements(int nvalue, int index)
        {
            if (index >= 0 && index < Length)
            {
                ...
                else if (nvalue > 0)
                {
                    ...
                    {
                        DoubleNode startNode = GetNodeByIndex(index - 1);
                        DoubleNode finishNode = GetNodeByIndex(index + nvalue);
